...
def run_onnx_benchmark(prompt, batch_size, seq_len, repeat):
    ...
    #so.log_severity_level = 0  # VERBOSE log
    #session = ort.InferenceSession(onnx_model, sess_options=so, providers=["CUDAExecutionProvider"])
    ...
    decodes = []
    vocab_size = 50257
    with nvtx.annotate("ONNX Benchmark", color="blue"):
